...
def home(request):
    ...
    products = Products.objects.select_related('category', 'subcategory').filter(available = True)
...
def get_subcategory(request, cat_slug, subcat_slug):
    ...
    subcategory = get_object_or_404(Subcategories.objects.filter(category__slug=cat_slug, is_public=True), slug=subcat_slug)
...
def product(request, cat_slug, subcat_slug, product_pk):
    ...
    product = get_object_or_404(Products.objects.select_related('category', 'subcategory').prefetch_related('images').filter(available=True, category__slug=cat_slug, subcategory__slug=subcat_slug), pk=product_pk)
    
    #Pagination for product.feedbacks
    feedback_list = product.feedbacks.filter(is_public=True)
    ...
    question_list = product.questions.filter(is_public=True).prefetch_related("answer")
    ...
    if 'recently_viewed' in request.session:
        ...
        products = Products.objects.filter(pk__in=request.session['recently_viewed'])[:5]
    ...
    context = get_user_context(
        ...
        other_sizes = Products.objects.filter(product=product.product).exclude(pk=product_pk).order_by('size'),
    )

    return render(request, 'topikstore/product.html', context)


def get_feedback(request, product_pk):
    if request.POST:
        form = FeedbackForm(request.POST)
        if form.is_valid():
            feedback = form.save(commit=False)
            feedback.product = Products.objects.get(pk = product_pk)
            form.save()
            return redirect(feedback.product.get_absolute_url())


def get_question(request, product_pk):
    if request.POST:
        form = QuestionForm(request.POST)
        if form.is_valid():
            question = form.save(commit=False)
            question.product = Products.objects.get(pk = product_pk)
            form.save()
